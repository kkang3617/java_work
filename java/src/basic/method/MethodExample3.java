...
/*
 ...
 5. 반환값이 있는 메서드는 호출문 자체가 하나의 값으로 처리될 수
    있기 때문에 반환값을 다른 변수에 대입할 수 있고, 다른 메서드의
    매개값으로도 처리될 수 있습니다.
...
public class MethodExample3 {
	...
	static double[] calcArrayTotal(int[] nums){   //아래 문제
		int total = 0;
		for(int n:nums) {
			total += n;
		}
	...
	static void multi(int n1, int n2 ) {  // 7번설명
		int result = n1 * n2;
		System.out.printf("%d x %d = %d\n", n1, n2, result);
	}
	
	static void divide(int n1, int n2) {  // 8번 설명
		if(n2 == 0) {
	...
	static public void main(String[] args) {
		...
		/*
		 -  반환값이 없는 void 타입의 메서드는 값이 돌아오지 않기 때문에
		 	실행 결과를 변수에 저장할 수 없고, 다른 메서드이 매개값으로도 
		 	사용이 불가능합니다.
