		...
		while(true) {
			...
			if(menu == 1) {
				...
				//(무한루프를 구현해서 사번 중복이 발생하면 다시 입력받기.)
					...
					while(true) {
						...
						for(int i=0; i<count; i++) {
							if(empNum.equals(userNums[i])) {
								...
								flag = true;  //중복된 사번이라 true로 바뀌면서
								break;        // 아래 if문 else로 가면서 
							}                 // 사번재입력이 나온다.
						}
						if(!flag) { //flag == false? -> 중복 발생 x
							userNums[count] = empNum; // count = n번째사원.
					...
					System.out.println(names[count] +"님의 정보가 정상저장되었습니다.");
					count++;
				...
				int i; //제어변수 선언  밑에 if에도 적용하기 위해 이쪽에서 변수 선언
				for(i=0; i<count; i++) {
					if(empNum.equals(userNums[i])) { // true 일치하는 사원정보가 있다
						System.out.printf("%s %s %d세 %s\n", userNums[i],names[i],
															ages[i],departments[i]);
						break;
					}
				...
				if(i == count) { 
					System.out.println("조회하신 사원정보는 없습니다.");
				}
				...
				for(int i=0; i<count; i++) {
					if(empNum.equals(userNums[i])) {
						...
						case "Y": case "y": case "ㅛ":
							if(i == userNums.length-1) {   // =99  99는 아래 for문
								userNums[i] = null;        // 안 돌아감. 그래서 위에 
								names[i] = null;           // if문으로 선언해줌
								ages[i] = 0;
								departments[i] = null;
							}
							for(int j=i; j<count-1; j++) {  // j<99 되서 98까지..
