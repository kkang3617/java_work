...
public class Point {
	...
	public void inputPoints(List<Student> students) {
		...
		students.add(stu);
		System.out.printf("%s 님의 성적 정보가 정상적으로 입력되었습니다.", stu.getName());
	...
	public void showAllPoints(List<Student> students) { //학생 객체들이 들어있는 리스트가 와야죠?
		 ...
		 */
		double totalAvg = 0.0; //반평균
		
		showPointUI();
		for(Student stu : students) {
			stu.outputStuInfo();
			totalAvg += stu.getAverage(); //학생 평균 모두 더하기
		}
		System.out.printf("우리반 평균: %.2f", totalAvg / students.size()); 
		 									//학생개인평균의 합 / 학생수 
	...
	public void modifyPoint(List<Student> students) {
		...
		for(Student stu : students) {
			if(stuNum.equals(stu.getStuId())) {
				System.out.printf("%s의 성적정보를 수정합니다.\n", stu.getName());
				...
				stu.calcTotAvgGrade(); //새롭게 계산
				System.out.println("성적정보 수정완료");
				flag = true;
				break;
	...
	public void deletePoint(List<Student> students) {
		...
		for(Student stu : students) {
			if(stuNum.equals(stu.getStuId())) {
				System.out.printf("%s 학생 정보를 삭제합니다\n", stu.getName());
				
				System.out.printf("[Y / N]: ");
				String answer = sc.next();
				
				if(answer.toUpperCase().equals("Y")) {  //toUppercase() :대문자변환
					students.remove(stu);
